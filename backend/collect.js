...
//
// data/memo.txt に書いた「配信者の発言」を1つずつAIに流し、
// 生成された視聴者コメントを dataset.js 経由で dataset.jsonl に保存する。
//
// 使い方（backend フォルダで実行）:
//   node collect.js                    … memo.txt を全部流す
//   node collect.js data/memo.txt 10   … 先頭10件だけ流す
//
...
const listPath = process.argv[2] || path.join(__dirname, "data", "memo.txt");
...
async function main() {
  ...
  console.log(`\n発言 ${memo.length} 件を流します。\n`);
 
  let ok = 0;
  let ng = 0;
 
  for (let i = 0; i < memo.length; i++) {
    const transcript = memo[i];
    const label = `[${i + 1}/${memo.length}]`;
